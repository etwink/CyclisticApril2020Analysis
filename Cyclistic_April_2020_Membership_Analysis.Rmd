...
ggplot(data=d5, aes(x=start_station_name, y=normalized_start_station_count, fill=member_casual))+
  geom_col(position = "dodge", color = "black", width = .75) + 
  coord_flip() +
  labs(title = "Normalized Favorite Start Locations", x="Start Station Name", y="Start Station Count")
```
...
ggplot(data=d7, aes(x=end_station_name, y=normalized_end_station_count, fill=member_casual))+
  geom_col(position = "dodge", color = "black", width = .75) + 
  coord_flip() +
  labs(title = "Normalized Favorite Destinations", x="End Station Name", y="End Station Count")
```
...
# Recommendations
## Electric Bikes
As shown in the "Ride Length" subsection, casual riders, on average, use a bike for 14 minutes longer than annual members. One strategy to convert casual riders to annual members would be to offer different bikes such as electric bikes or battery assisted bikes to annual members. These bikes would be appealing to riders that have longer ride times as these bikes reduce the physical strain of long rides. Certain bike docks could be converted to allow the charging of these electric bikes so that riders will always have a fully charged electric bike when they want to use one.

## Member Only Docking Stations
Another strategy would be to add more docking stations near popular locations but make them only accessible to annual members. A few possible locations would be near the Wabash Ave & Grand Ave station, Stockton Dr & Wrightwood Ave station, Desplaines St & Kinzie St station, Dearborn St & Erie St station, and Broadway & Barry Ave station. Multiple locations along Wells St as well as Clark St are also popular and may benefit from adding annual member only stations as well. These popular stations can be seen in the "Favorite Start Locations" and "Favorite End Destinations" sections.

## Surge Pricing
The final strategy proposal, which will most likely be unpopular among casual riders, is to impose "Surge Pricing" on casual riders, which is what many ride-share companies, like Uber and Lyft, employ to hike prices in situations of high demand of low supply. In the "Start Time Analysis" section, it is shown that the peak ridership for both annual members and casual riders is between 13:00 (1:00PM) and 18:00 (6:00PM). During those times, single-ride passes could be sold at an elevated price. This may persuade casual riders to become annual members if they find themselves using single-ride passes very often during this time window.
